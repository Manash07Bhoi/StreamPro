...
class VideoRepository {
  ...
  Future<List<VideoEntity>> searchVideos(String query,
      {bool ageRestricted = false}) async {
    final box = Hive.box<VideoEntity>(videoBoxName);
    return box.values.where((video) {
  ...
  Future<List<VideoEntity>> getRelatedVideos(
      String category, String currentVideoId) async {
    final box = Hive.box<VideoEntity>(videoBoxName);
    return box.values
        .where((v) => v.category == category && v.id != currentVideoId)
        .take(5)
        .toList();
  }
  ...
  List<VideoEntity> _generateSampleVideos() {
    ...
    for (var entry in categoryCounts.entries) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        final durationSecs =
            random.nextInt(45 * 60 - 3 * 60) + 3 * 60; // 3 to 45 mins
        ...
        list.add(VideoEntity(
          ...
          embedCode:
              '<iframe width="100%" height="100%" src="https://www.youtube.com/embed/dQw4w9WgXcQ?autoplay=1&controls=0" frameborder="0" allowfullscreen></iframe>',
          category: category,
          description:
              'A compelling 1-2 sentence description of this $category video. Experience premium entertainment.',
          channelName: 'StreamPro $category',
          channelAvatar:
              'https://picsum.photos/seed/channel_${category}_$i/100/100',
          ...
          uploadedAt: DateTime.now()
              .subtract(Duration(days: random.nextInt(365) + 1))
              .toIso8601String(),
          tags: ['4K', 'HD', category],
